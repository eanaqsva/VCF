const CACHE = 'vcf-v16-patch';
const ASSETS = ['./questions.js','./review-patch-v16.js','./manifest.webmanifest','./icon-192.png','./icon-512.png','./apple-touch-icon.png'];
...
self.addEventListener('fetch', event => {
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request, {cache:'no-store'}).then(async response => {
      ...
      html = html.replace(/<script src="\.\/review-patch-v15\.js"><\/script>/g, '');
      if (!html.includes('review-patch-v16.js')) html = html.replace('</body>', '<script src="./review-patch-v16.js"></script></body>');
